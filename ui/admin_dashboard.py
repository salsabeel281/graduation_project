...
class StatusBar(QFrame):
    def __init__(self, parent=None):
        ...
        # ✅ ADD THIS - Create the user name label
        ...
        user_layout.addWidget(self.user_name)  # ✅ ADD THIS LINE
...
# At the top of AdminDashboard class, fix the __init__ method:

class AdminDashboard(QMainWindow):
    def __init__(self, token=None, sidebar=None):
        super().__init__()
        self.token = token
        self.external_sidebar = sidebar  # ✅ ADD THIS LINE
        ...
        # ✅ Load real users and frozen users
        self._load_real_users()
        
        # ✅ Load real threats (MOVE THIS OUTSIDE _load_real_users)
        self._load_real_threats()
    ...
    # -------------------- Data generators --------------------
    def _load_real_users(self):
        self.users_data = []
        self.frozen_users_data = []
        if not self.service:
            return
        all_users = self.service.get_all_users()
        frozen_users = self.service.get_frozen_users()
        frozen_ids = {str(u['id']) for u in frozen_users}
        for u in all_users:
            uid = str(u.get('id', ''))
    ...
    def _load_real_threats(self):
        self.threats_data = []
        if not self.service:
            return
        raw = self.service.get_latest_threats()
        for t in raw:
            self.threats_data.append({
                'time': str(t.get('timestamp', 'N/A')),
                'user': t.get('user_id', 'Unknown'),
                'type': t.get('alerts', 'Unknown'),
                'severity': t.get('status', 'Low'),
    ...
    def _setup_dashboard(self):
        ...
        stats = self.service.get_system_dashboard() if self.service else {}
        total = stats.get('total_users', len(self.all_users_data))
        active = stats.get('active_users', 0)
        total_threats = stats.get('total_risks_detected', 0)
    ...
    def _unfreeze_user(self, user):
        ...
        f"Unfreeze {user['name']}?", QMessageBox.Yes | QMessageBox.No)
        
        if reply == QMessageBox.Yes:
            if self.service:
                success = self.service.unfreeze_user(user['id'])
            if not success:
                QMessageBox.warning(self, "Error", "Failed to unfreeze user on server.")
                return
        ...
        self.users_data.append({
            'id': user['id'], 'name': user['name'], 'email': user['email'],
            'department': user['department'], 'risk_score': 0, 'threat_level': 0,
            'status': 'Active', 'last_active': datetime.now().strftime("%Y-%m-%d %H:%M"),
            'alerts_count': 0
        })
    ...
    def _setup_profile(self):
        ...
        admin_department = "Security Operations"
        admin_role = "Security Administrator"
        
        if self.admin_profile:
            first = self.admin_profile.get('first_name', '')
            last = self.admin_profile.get('last_name', '')
            admin_name = f"{first} {last}".strip() or "Admin User"
            admin_email = self.admin_profile.get('email', admin_email)
            admin_department = self.admin_profile.get('department', admin_department)
            if self.admin_profile.get('account_type') == 'Administrator':
                admin_role = "Security Administrator"
            else:
                admin_role = self.admin_profile.get('account_type', admin_role)
        
        # Get initials
        parts = admin_name.strip().split()
        initials = (parts[0][0] + parts[-1][0]).upper() if len(parts) >= 2 else admin_name[:2].upper()
        ...
        avatar_text.setFont(QFont("Segoe UI", 28, QFont.Bold))
        ...
        # Role
        role_label = QLabel(admin_role)
        role_label.setFont(QFont("Segoe UI", 13))
        role_label.setAlignment(Qt.AlignCenter)
        role_label.setStyleSheet("color: #4A6FFF; font-weight: 500;")
        ...
        active_users = len([u for u in self.users_data if u['status'] == 'Active'])
        frozen_users = len(self.frozen_users_data)
        ...
        details = [
            ...
            ("❄️ Frozen Users", str(frozen_users)),
